...
function modifyHeader() {
  ...
  var oldHeader, headerDiv, logoA, searchDiv, simpDiv, i = 0, headerImg, sL = 0, cat, libpn,
    ...
    langURL, linkEle, i1 = 0, searchDiv1, spanSearch, productSearch, metas, k = 0, divLogo, solnOption;
  function setBase() {
   searchURL = window.location.hostname + "/";
   if (searchURL.indexOf("pdb-stage") !== -1) {
  ...
  headernav.className = "sign-in-out";
  ...
  liSign = document.createElement("span");
  if (user === "") {
    ...
    headernav.appendChild(liSign);
    //ulSign.appendChild(liSign);
  } else {
    liSign.appendChild(document.createTextNode(user));
    aSign = document.createElement("a");
    aSign.href = "#";
    aSign.appendChild(document.createTextNode("Sign out"));
    aSign.onclick = function () { return function(){sso_sign_out();}}();
    headernav.appendChild(liSign);
    headernav.appendChild(aSign);
  }
  ...
  if (document.getElementById("searchbody") === undefined || document.getElementById("searchbody") === null) {
    ...
    searchLabel.appendChild(document.createTextNode("Search entry"));
    ...
    if (prodList.products !== undefined ) {
      ...
      solnOption = document.createElement("option");
      solnOption.value = "solutions";
      solnOption.appendChild(document.createTextNode("Solutions"));
      prodSelect.appendChild(solnOption);
    }
    if(listOfCategories.length > 1) {
...
document.getElementById("searchpage").addEventListener("submit", function(event){
    ...
    var prod = document.getElementById("selectproduct");
  if(prod.value !== null && prod.value === 'solutions') {
    window.location = "/en/solutions/solutions.html?search="+document.getElementById("searchField").value;
   }
   else {
    document.getElementById("searchpage").action = "/apps/search/search.jsp";
    window.location = "/apps/search/search.jsp?q="+document.getElementById("searchField").value+"&category="+prod.value;
   }
    
});
}
addLoadEvent(solnURL);
function addNavigation() {
    "use strict";
    var leftNav, ul, li, a, homeA, homeLi, homeImg, content, body, createNav = false, i = 0, showCont,
      hideCont, divs, divNav = false, j = 0, solnLi, solnA;
    ...
    if (createNav && !divNav) {
        ...
        homeImg = document.createElement("span");
        homeImg.className = "JetFW-caret02-w_16";
        homeImg.setAttribute("aria-hidden","true");
        homeA.appendChild(homeImg);
        homeA.appendChild(document.createTextNode(" Home"));
        homeLi.appendChild(homeA);
        ul.appendChild(homeLi);
        solnLi = document.createElement("li");
        solnA = document.createElement("a");
        solnA.id = "Solutions";
        solnA.href = baseurl + "solutions/";
        solnA.appendChild(document.createTextNode("Solutions"));
        solnLi.appendChild(solnA);
        ul.appendChild(solnLi);
         if (prodList.products !== undefined) {
...
function addTrustE() {
  "use strict";
  var siteUrl = "//consent.truste.com/notice?domain=oracle.com&c=teconsent&js=bb&noticeType=bb&text=true";
  function loadScript(url) {
    var script = document.createElement('script');
    script.type = 'application/javascript';
    script.src = url;
...
function modifyFooter () {
  ...
  var oldFooter, footerDiv, body, divs, i = 0, footerExist = false, footDiv, cpyrSpan,
    ulFooterLinks, ulFli1, aF1, ulFli2, aF2, ulFli3, aF3, ulFli4, aF4, ulFli5, aF5, ulFli6, aF6, langURL,
    footList, mainDiv, ulLang, langLi, langA, langImg, langSpan, enLang, enLangA, langShad,
    linkEle, sL = 0, body, i1 = 0, formLang, langSub, subLabel, d = new Date();
  body = document.getElementsByTagName('body')[0];
  ...
  mainDiv.className = "language";
  //mainDiv.className = "glyph";
  formLang = document.createElement("form");
  formLang.id = "region_form";
  ulLang = document.createElement("select");
  ulLang.id = "region";
  ulLang.className = "SELECTLRG SELECTLRGBG";
  ulLang.name = "region";
  langLi = document.createElement("option");
  langLi.setAttribute("selected", "selected");
  langLi.setAttribute("lang", "en");
  langLi.setAttribute("title", "Choose Language");
  langLi.appendChild(document.createTextNode("Select Language"));
  ...
  if (langURL === undefined || langURL === "") {
    enLang = document.createElement("option");
    enLang.value = "#";
    enLang.appendChild(document.createTextNode("English"));
    ulLang.appendChild(enLang);
  } else {
     ...
     if (langErr === 1) {
        ...
        for (var keys in langList) {
          ...
          if (langList[keys] !== undefined) {
            enLang = document.createElement("option");
            enLang.value = langList[keys];
            if (langCode[keys]) {
              enLang.appendChild(document.createTextNode(langCode[keys]));
            } else {
              enLang.appendChild(document.createTextNode(keys));
            }
            ulLang.appendChild(enLang);
            sL += 1;
          }
  ...
  ulLang.appendChild(langLi);
  formLang.appendChild(ulLang);
  langSub = document.createElement("input");
  langSub.id = "language-submit";
  langSub.className = "button";
  langSub.setAttribute("type", "submit");
  langSub.value = "Go";
  formLang.appendChild(langSub);
  subLabel = document. createElement("label");
  subLabel.className = "obscure";
  subLabel.setAttribute("for", "region");
  subLabel.appendChild(document.createTextNode("Language Selection"));
  formLang.appendChild(subLabel);
  mainDiv.appendChild(formLang);
  ...
  aF6.id = "teconsent";
   aF6.consent = "null";
  //aF6.appendChild(document.createTextNode("Cookie Preferences"));
  ...
  cpyrSpan = document.createElement("div");
  ...
  if (oldFooter !== undefined && oldFooter !== null) {
    ...
    oldFooter = document.getElementsByTagName('footer')[0];
    oldFooter.className = "footer-container clearboth";
    oldFooter.appendChild(footDiv);
  } else {
...
function createComments() {
  ...
  if (fbForm !== null && typeof fbForm !== 'undefined') {
    l10n = {
      ...
      subject : "Feedback subject entry",
      from : "From",
      anonymous : "Anonymous",
      or : " (or ",
      sign_in : "Sign In",
    ...
    userCommentsBodyLabel.appendChild(document.createTextNode("Enter feedback comments"));
    userCommentsBodyLabel.className = "obscure";
    ...
    outDiv.appendChild(subject);
    outDiv.appendChild(userCommentsBodyLabel);
  ...
  if (document.getElementById("searchbody") === undefined || document.getElementById("searchbody") === null) {
    ...
    if (cat === null || (cat !== undefined && cat !== null && cat.innerHTML.indexOf("Cloud") === -1)) {
      for (i = 0; i < aList.length; i += 1) {
        ...
        setBaseUrl();
        if (aList[i].href && r.test(aList[i].href) && !aList[i].getAttribute("target") && ((classInfo !== '' && classInfo.indexOf("newwindow") === -1 && classInfo.indexOf("various") === -1) || classInfo === '') && !((aList[i].parentNode.parentNode.parentNode !== null &&  aList[i].parentNode.parentNode.parentNode.nodeType === 1 && aList[i].parentNode.parentNode.parentNode.getAttribute("class") && aList[i].parentNode.parentNode.parentNode.className.indexOf("footer-list") !== -1)) && aList[i].href.indexOf("/pls/topic") === -1) {
